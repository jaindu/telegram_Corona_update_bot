...
TOKEN = '1192282997:AAF3oH7OcADUhlVnIHsYM2egGB6RBzFKkho'
URL = 'https://api.telegram.org/bot{}/'.format(TOKEN)

def staat(qq):
  url = "https://api.telegram.org/bot1192282997:AAF3oH7OcADUhlVnIHsYM2egGB6RBzFKkho/sendphoto"
  data = {
    "chat_id": str(qq),
...
def corona(updates):
    ...
    for update in updates['result']:
        ...
        try:
            ...
            if update['message'][
                    'text'] == '/corona@coronasrilankabot' or update[
                        'message']['text'] == '/corona':
                r = requests.get(
                ...
                local_active_cases = str(
                    jsondata['data']['local_active_cases'])
                ...
                local_total_number_of_individuals_in_hospitals = str(
                    jsondata['data']
                    ['local_total_number_of_individuals_in_hospitals'])
                global_new_cases = str(jsondata['data']['global_new_cases'])
                global_total_cases = str(
                    jsondata['data']['global_total_cases'])
                ...
                textt = str(
                    '<b>CURRENT SITUATION</b>' + '%0D%0A' + '%0D%0A' + '<u>' +
                    update_date_time + ' වන විට</u>' + '%0D%0A' + '%0D%0A' +
                    '<u>ශ්‍රී ලංකාවේ තත්ත්වය</u>' + '%0D%0A' + '%0D%0A' +
                    'තහවුරු කරනලද රෝගීන් සංඛ්‍යාව(සමුච්චිත) = ' +
                    local_total_cases + '%0D%0A' +
                    'ප්‍රතිකාර ලබන රෝගීන් සංඛ්‍යාව = ' + local_active_cases +
                    '%0D%0A' + 'නව රෝගීන් සංඛ්‍යාව = ' + local_new_cases +
                    '%0D%0A' +
                    'දැනට රෝහල්වල විමර්ශන යටතේ සිටින පුද්ගලයින් = ' +
                    local_total_number_of_individuals_in_hospitals + '%0D%0A' +
                    'සුවය ලබා පිටව ගිය සංඛ්‍යාව = ' + local_recovered +
                    '%0D%0A' + 'මරණ සංඛ්‍යාව = ' + local_deaths + '%0D%0A' +
                    '%0D%0A' + '<u>ලොව පුරා තත්ත්වය</u>' + '%0D%0A' +
                    '%0D%0A' + 'තහවුරු කරනලද රෝගීන් සංඛ්‍යාව(සමුච්චිත) = ' +
                    global_total_cases + '%0D%0A' + 'නව රෝගීන් සංඛ්‍යාව = ' +
                    global_new_cases + '%0D%0A' + 'මරණ සංඛ්‍යාව = ' +
                    global_deaths + '%0D%0A' + 'සුවය ලැබූ සංඛ්‍යාව = ' +
                    global_recovered + '%0D%0A' + '%0D%0A' + '%0D%0A' +
                    'සියලු තොරතුරු රජයේ සහ පිලිගත් මුලාශ්‍ර මගිනි' + '%0D%0A' +
                    '@coronasrilankabot')
                ...
                print('hi')
                    ...
                    'text'] == '/start@coronasrilankabot' or update['message'][
                        'text'] == '/start':
